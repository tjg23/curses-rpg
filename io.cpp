...
void io_display()
{
  ...
  * not otherwise used.                                                 */
  ...
  mvprintw(23, 1, "PC position is (%2d,%2d) on map %d%cx%d%c [%c]",
           world.pc.pos[dim_x],
           world.pc.pos[dim_y],
           ...
           geo_symb[world.cur_map->geotype]);
...
int32_t battle_menu(int mode, npc* n = NULL, pokemon* w = NULL) {
  ...
  int pc_move = -1;
	...
	pokemon& f = mode ? *w : *(n->buddy[0]);
	int n_lives = mode ? 1 : n->num_buddies;
